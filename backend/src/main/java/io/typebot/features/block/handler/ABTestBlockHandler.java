...
import io.typebot.features.block.model.options.ABTestBlockOptions;
import io.typebot.features.block.model.options.TestVariant;
import io.typebot.features.block.model.options.SelectionMode;
...
public class ABTestBlockHandler implements BlockHandler {
    ...
    private TestVariant selectVariant(List<TestVariant> variants, SelectionMode mode, String seed) {
    ...
    private TestVariant selectSequentialVariant(List<TestVariant> variants, String blockId) {
        AtomicInteger counter = sequentialCounters.computeIfAbsent(blockId, k -> new AtomicInteger(0));
